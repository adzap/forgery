...
describe Forgery::Internet do
  ...
  it "should return a top level domain" do
    Forgery::Tool.dictionaries[:top_level_domains].should include(Forgery::Internet.top_level_domain)
  end
  ...
  describe ".domain_name" do
    it "should return a domain name that contains a lowercase company name" do
      ...
      Forgery::Tool.dictionaries[:company_names].any?{ |cn| cn =~ /#{downcase_company_name}/i }
    end

    it "should return a domain name that contains a top level domain" do
      Forgery::Tool.dictionaries[:top_level_domains].should include(Forgery::Internet.domain_name.split('.').last)
    end
  end

  describe ".email_address" do
    it "should match the email format" do
      Forgery::Internet.email_address.should match(/.+@.+\.(#{Forgery::Tool.dictionaries[:top_level_domains].join("|")})/)
    end
  end

  describe ".cctld" do
    it "should return a country-code top level domain" do
      Forgery::Tool.dictionaries[:country_code_top_level_domains].should include(Forgery::Internet.cctld)
    end
